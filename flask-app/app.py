...
def save_node():
    try:
        ...
        session.execute(
            """
            INSERT INTO nodes (customer_email, node_id, type, position, data, style)
            VALUES (%s, %s, %s, %s, %s, %s)
            """,
            (node['customer_email'], node['id'], node['type'], node['position'], node['data'], node['style'])
        )
